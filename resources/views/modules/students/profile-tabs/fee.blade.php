<div class="space-y-5">
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Pending Amount</p>
            <p class="mt-2 text-2xl font-semibold text-rose-700">Rs. {{ number_format((float) ($feeStats['pending_amount'] ?? 0), 2) }}</p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p class="text-xs font-medium uppercase tracking-wide text-slate-500">Overdue Challans</p>
            <p class="mt-2 text-2xl font-semibold text-amber-700">{{ (int) ($feeStats['overdue_challans'] ?? 0) }}</p>
        </div>
    </div>

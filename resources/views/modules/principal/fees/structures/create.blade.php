        ...
        <div class="mx-auto max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="p-5 sm:p-6">
                    <form method="POST" action="{{ route('principal.fees.structures.store') }}" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        @csrf
                        ...
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block min-h-11 w-full" value="{{ old('title') }}" required placeholder="e.g. Monthly Tuition Fee" />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="fee_type" name="fee_type" required class="mt-1 block min-h-11 w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                @foreach($feeTypes as $feeType)
                                    <option value="{{ $feeType }}" @selected(old('fee_type') === $feeType)>{{ ucwords(str_replace('_', ' ', $feeType)) }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="md:col-span-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <label class="inline-flex min-h-11 items-center gap-2 rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-700">
                                <input type="checkbox" name="is_monthly" value="1" @checked(old('is_monthly')) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                Monthly recurring fee
                            </label>
                            <label class="inline-flex min-h-11 items-center gap-2 rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-700">
                                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', '1') === '1') class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                Active
                            </label>
                        </div>

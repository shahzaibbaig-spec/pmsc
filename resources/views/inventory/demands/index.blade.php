<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-slate-900">Inventory Demand Reviews</h2>
            <p class="mt-1 text-sm text-slate-500">Review teacher stationery demands and proceed to fulfillment.</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                <form method="GET" action="{{ route('inventory.demands.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                    <div class="md:col-span-2">
                        ...
                        <input
                            ...
                            name="search"
                            value="{{ $search }}"
                            placeholder="Teacher name or session"
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($demands as $demand)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-slate-700">{{ $demand->teacher?->user?->name ?? '-' }}</td>
                                ...
                                <td class="px-4 py-3 text-sm text-slate-700">{{ str_replace('_', ' ', $demand->status) }}</td>
                                <td class="px-4 py-3 text-right">
                                    <a
                                        href="{{ route('inventory.demands.show', $demand) }}"

<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Warden review panel for teacher-submitted discipline reports.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('warden.class-discipline-reports.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div>
                ...
                <div class="md:col-span-4 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('warden.class-discipline-reports.index') }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </section>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($reports as $report)
                            <tr>
                                ...
                                <td class="px-4 py-4 text-sm text-slate-700">{{ \Illuminate\Support\Str::limit($report->auto_message, 100) }}</td>
                                ...
                                <td class="px-4 py-4 text-sm">
                                    <div class="flex flex-col gap-2">
                                        <a href="{{ route('warden.class-discipline-reports.show', $report) }}" class="inline-flex min-h-9 items-center rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                                            View
                                        </a>
                                        @if ($report->status === 'open')
                                            <form method="POST" action="{{ route('warden.class-discipline-reports.acknowledge', $report) }}" class="flex flex-col gap-2">
                                                @csrf
                                                <input type="text" name="warden_remarks" placeholder="Warden remarks (optional)" class="min-h-9 rounded-lg border-slate-300 text-xs">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-4 py-10 text-center text-sm text-slate-500">No reports found for selected filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

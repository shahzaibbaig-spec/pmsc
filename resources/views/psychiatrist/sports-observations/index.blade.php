<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900">Psychiatrist Sports Observation Reviews</h2>
                <p class="mt-1 text-sm text-slate-500">Viewer access to sports observations with psychologist feedback per case.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('psychiatrist.sports-observations.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div>
                ...
                <div class="md:col-span-4 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('psychiatrist.sports-observations.index') }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </section>
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-4 py-3">
                <h3 class="text-base font-semibold text-slate-900">Sports Cases</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-4 text-sm">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold {{ $observation->status === 'resolved' ? 'bg-emerald-100 text-emerald-700' : ($observation->status === 'acknowledged' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700') }}">
                                        {{ ucfirst((string) $observation->status) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-4 text-sm">
                                    <a href="{{ route('psychiatrist.sports-observations.show', $observation) }}" class="inline-flex min-h-9 items-center rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                                        Review
                                    </a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-10 text-center text-sm text-slate-500">No sports observations found for selected filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

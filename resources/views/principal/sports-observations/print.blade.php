...
<body>
    @php
        ...
        $selectedTeacher = collect($sports_teachers ?? [])->firstWhere('id', (int) ($filters['sports_teacher_id'] ?? 0));
    ...
    <div class="shell">
        <div class="toolbar">
            ...
            <a href="{{ route('principal.sports-observations.daily', request()->query()) }}" class="btn">Back</a>
        </div>

        <section class="sheet">
            <header class="header">
                <h1 class="title">Daily Sports Hygiene &amp; Discipline Report</h1>
                <p class="subtitle">Consolidated observations submitted by Sports Teacher for Principal/Warden follow-up.</p>
            </header>
            ...
            <div class="meta">
                ...
                <div><strong>Sports Teacher:</strong> {{ $teacherLabel }}</div>
                <div><strong>Status:</strong> {{ ucfirst((string) ($filters['status'] ?? 'all')) }}</div>
                <div><strong>Total Observations:</strong> {{ number_format(is_countable($observations) ? count($observations) : 0) }}</div>
            ...
            <table>
                ...
                <tbody>
                    @forelse ($observations as $index => $observation)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ optional($observation->observation_date)->format('d M Y') ?: '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" style="text-align:center;padding:22px;color:#475569;">No observations found for selected filters.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900">Sports Observations Monitoring</h2>
                <p class="mt-1 text-sm text-slate-500">Principal/Admin panel for hygiene and sports discipline observations.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('principal.sports-observations.daily', array_merge(request()->query(), ['date' => request('date', now()->toDateString())])) }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    Daily View
                </a>
                <a href="{{ route('principal.sports-observations.print', array_merge(request()->query(), ['date' => request('date', now()->toDateString())])) }}" target="_blank" class="inline-flex min-h-11 items-center rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                    Print Report
                </a>
            </div>
        </div>
    ...
    <div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('principal.sports-observations.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div>
                ...
                <div class="md:col-span-4 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('principal.sports-observations.index') }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </section>
        ...
        <section class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
            ...
            <article class="rounded-2xl border border-rose-200 bg-rose-50 p-4 shadow-sm"><p class="text-xs font-semibold uppercase text-rose-700">Serious</p><p class="mt-2 text-2xl font-semibold text-rose-800">{{ number_format((int) ($cards['serious'] ?? 0)) }}</p></article>
            <article class="rounded-2xl border border-cyan-200 bg-cyan-50 p-4 shadow-sm"><p class="text-xs font-semibold uppercase text-cyan-700">Repeated</p><p class="mt-2 text-2xl font-semibold text-cyan-800">{{ number_format((int) ($cards['repeated'] ?? 0)) }}</p></article>
        </section>

        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-4 py-3">
                <h3 class="text-base font-semibold text-slate-900">Observations</h3>
            </div>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-blue-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-blue-700">Auto Message</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        @forelse ($observations as $index => $observation)
                            <tr>
                                ...
                                <td class="px-4 py-4 text-sm text-slate-700">{{ \Illuminate\Support\Str::limit((string) $observation->auto_message, 120) }}</td>
        ...
        <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3"><h3 class="text-base font-semibold text-slate-900">Issue-wise Summary</h3></div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50"><tr><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Issue</th><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Total</th></tr></thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($issue_summary as $row)
                                <tr><td class="px-4 py-3 text-sm text-slate-700">{{ $row['issue_label'] }}</td><td class="px-4 py-3 text-sm font-semibold text-slate-900">{{ number_format($row['total']) }}</td></tr>
                            @empty
                                <tr><td colspan="2" class="px-4 py-8 text-center text-sm text-slate-500">No data.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3"><h3 class="text-base font-semibold text-slate-900">Class-wise Summary</h3></div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50"><tr><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Class</th><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Total</th></tr></thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($class_summary as $row)
                                <tr><td class="px-4 py-3 text-sm text-slate-700">{{ $row['class_name'] ?: '-' }}</td><td class="px-4 py-3 text-sm font-semibold text-slate-900">{{ number_format($row['total']) }}</td></tr>
                            @empty
                                <tr><td colspan="2" class="px-4 py-8 text-center text-sm text-slate-500">No data.</td></tr>
                            @endforelse
                        </tbody>
        ...
        <section class="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3"><h3 class="text-base font-semibold text-slate-900">Repeated Students Report</h3></div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50"><tr><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Student</th><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Class</th><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Total</th></tr></thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($repeated_students as $row)
                                <tr>
                                    <td class="px-4 py-3 text-sm text-slate-700">{{ $row['student_name'] }}<p class="text-xs text-slate-500">{{ $row['admission_no'] }}</p></td>
                                    <td class="px-4 py-3 text-sm text-slate-700">{{ $row['class_name'] }}</td>
                                    <td class="px-4 py-3 text-sm font-semibold text-slate-900">{{ number_format($row['total']) }}</td>
                                </tr>
                            @empty
                                <tr><td colspan="3" class="px-4 py-8 text-center text-sm text-slate-500">No repeated students in selected scope.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3"><h3 class="text-base font-semibold text-slate-900">Resolved / Unresolved Report</h3></div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50"><tr><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Status</th><th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Total</th></tr></thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($status_summary as $row)
                                <tr><td class="px-4 py-3 text-sm text-slate-700">{{ ucfirst($row['status']) }}</td><td class="px-4 py-3 text-sm font-semibold text-slate-900">{{ number_format($row['total']) }}</td></tr>
                            @empty
                                <tr><td colspan="2" class="px-4 py-8 text-center text-sm text-slate-500">No data.</td></tr>
                            @endforelse
                        </tbody>

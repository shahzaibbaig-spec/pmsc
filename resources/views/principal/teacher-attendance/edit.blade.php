<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900">Edit Teacher Attendance</h2>
                <p class="mt-1 text-sm text-slate-500">Update an existing attendance record. Manual changes remain the authoritative entry for the date.</p>
            </div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('principal.teacher-attendance.update', $attendance) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="teacher_id" name="teacher_id" required class="mt-1 block min-h-11 w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-slate-500 focus:ring-slate-500">
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" @selected((string) old('teacher_id', $attendance->teacher_id) === (string) $teacher->id)>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                required
                                value="{{ old('attendance_date', optional($attendance->attendance_date)->toDateString()) }}"
                    ...
                    <div>
                        ...
                        <textarea id="remarks" name="remarks" rows="4" maxlength="1000" class="mt-1 block w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-slate-500 focus:ring-slate-500">{{ old('remarks', $attendance->remarks) }}</textarea>
                    </div>

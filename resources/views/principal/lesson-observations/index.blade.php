<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900">Lesson Observations</h2>
                <p class="mt-1 text-sm text-slate-500">Principal/Admin lesson observation records and scoring summary.</p>
            </div>
            @can('conduct_lesson_observation')
                <a href="{{ route('principal.lesson-observations.create') }}" class="inline-flex min-h-11 items-center rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                    New Observation
                </a>
            @endcan
        </div>
    ...
    <div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('principal.lesson-observations.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-4">
                <div>
                ...
                <div class="md:col-span-4 flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('principal.lesson-observations.index') }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </section>
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-blue-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-blue-700">Judgment</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        @forelse ($observations as $index => $observation)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-slate-700">{{ $observation->overall_judgment ?: '-' }}</td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex flex-wrap gap-2">
                                        <a href="{{ route('principal.lesson-observations.show', $observation->id) }}" class="inline-flex min-h-9 items-center rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50">View</a>
                                        @can('print_observations')
                                            <a target="_blank" href="{{ route('principal.lesson-observations.print', $observation->id) }}" class="inline-flex min-h-9 items-center rounded-lg border border-blue-300 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100">Print</a>
                                        @endcan
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="px-4 py-10 text-center text-sm text-slate-500">No lesson observations found for selected filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

<x-app-layout>
    ...
    <div class="space-y-6">
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('principal.analytics.dashboard.index') }}" class="grid grid-cols-1 gap-4 md:grid-cols-6">
                <div>
                ...
                <div>
                    <label for="exam" class="block text-xs font-semibold uppercase tracking-wide text-slate-500">Exam</label>
                    <select id="exam" name="exam" class="mt-1 block min-h-11 w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="" @selected(empty($selectedExam))>All Exams</option>
                        <option value="class_test" @selected(($selectedExam ?? null) === 'class_test')>Class Test</option>
                        <option value="bimonthly_test" @selected(($selectedExam ?? null) === 'bimonthly_test')>Bimonthly Test</option>
                        <option value="first_term" @selected(($selectedExam ?? null) === 'first_term')>1st Term</option>
                        <option value="final_term" @selected(($selectedExam ?? null) === 'final_term')>Final Term</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-3 flex flex-wrap items-end gap-3">
                    ...
                    <a href="{{ route('principal.analytics.export.pdf', $exportQuery) }}" class="inline-flex min-h-11 items-center rounded-xl border border-rose-300 bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-100">
                        Export PDF
                    </a>
                    <a href="{{ route('principal.analytics.export.excel', $exportQuery) }}" class="inline-flex min-h-11 items-center rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-100">
                        Export Excel
                    </a>
                    <a href="{{ route('principal.analytics.export.board-summary-pdf', $exportQuery) }}" class="inline-flex min-h-11 items-center rounded-xl border border-indigo-300 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">
                        Board Summary PDF
                    </a>

    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <form method="GET" action="{{ route('principal.daily-diary.completion-report') }}" class="grid grid-cols-1 gap-4 md:grid-cols-5">
                    <div>
            ...
            @elseif ($missingPostings > 0)
                <section class="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm text-rose-700">
                    <p class="font-semibold">Alert: {{ number_format($missingPostings) }} expected diary posting(s) are pending.</p>
                    <p class="mt-1">Coordinate with concerned teachers to complete the remaining entries.</p>
                </section>
            @elseif ($completionPercentage >= 100)
                <section class="rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-700">
                    Excellent: all expected diary postings are completed for this scope.
                </section>
            @endif
            ...
            <section class="flex flex-wrap gap-2">
                <a href="#missing-entries" class="inline-flex min-h-11 items-center rounded-xl border border-rose-300 bg-white px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50">
                    View Missing Entries
                </a>
                <a
                    href="{{ route('principal.daily-diary.index', ['session' => $filters['session'], 'date' => $filters['date'], 'class_id' => $filters['class_id'], 'teacher_id' => $filters['teacher_id'], 'subject_id' => $filters['subject_id']]) }}"
                    class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                >
            ...
            <section id="missing-entries" class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-4 py-3">
                    <h3 class="text-base font-semibold text-slate-900">Class-wise Completion</h3>
                    <p class="mt-1 text-xs text-slate-500">Coverage summary by class for the selected date and session.</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Class</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Expected</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Posted</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Missing</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Completion %</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($classwiseCompletion as $row)
                                <tr>
                                    <td class="px-4 py-4 text-sm font-semibold text-slate-900">{{ $row['class_name'] }}</td>
                                    <td class="px-4 py-4 text-sm text-slate-700">{{ number_format((int) $row['expected_postings']) }}</td>
                                    <td class="px-4 py-4 text-sm text-emerald-700">{{ number_format((int) $row['posted']) }}</td>
                                    <td class="px-4 py-4 text-sm {{ (int) $row['missing'] > 0 ? 'text-rose-700' : 'text-emerald-700' }}">{{ number_format((int) $row['missing']) }}</td>
                                    <td class="px-4 py-4 text-sm font-semibold {{ (float) $row['completion_percentage'] >= 100 ? 'text-emerald-700' : 'text-indigo-700' }}">
                                        {{ number_format((float) $row['completion_percentage'], 2) }}%
                                    </td>
                                </tr>
                            @empty
                                <tr>

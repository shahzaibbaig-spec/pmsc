    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">Submitted report details and follow-up status.</p>
            </div>
    ...
    <div class="mx-auto max-w-5xl space-y-6 py-8">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-4 grid grid-cols-1 gap-3 text-sm text-slate-700 md:grid-cols-2">
                ...
                <p><span class="font-semibold text-slate-900">Reported By:</span> {{ $report->teacher?->name ?? '-' }}</p>
                <p><span class="font-semibold text-slate-900">Report Date:</span> {{ optional($report->report_date)->format('d M Y') ?: '-' }}</p>

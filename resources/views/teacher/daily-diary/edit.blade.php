            ...
            <div>
                ...
                <div class="mt-3 inline-flex rounded-xl border border-slate-200 bg-white p-1 text-sm">
                    @can('create_daily_diary')
                        <a href="{{ route('teacher.daily-diary.create', ['session' => $selectedSession]) }}" class="rounded-lg px-3 py-1.5 font-semibold text-slate-600 hover:bg-slate-100">Post Diary Entry</a>
                    @endcan
                    @can('view_own_daily_diary_entries')
                        <a href="{{ route('teacher.daily-diary.my-entries', ['session' => $selectedSession]) }}" class="rounded-lg bg-slate-900 px-3 py-1.5 font-semibold text-white">My Diary Entries</a>
                    @endcan
                </div>
            </div>
            <a

<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900">My Diary Entries</h2>
                <p class="mt-1 text-sm text-slate-500">Review your submitted diary entries and verify what students can see.</p>
                <div class="mt-3 inline-flex rounded-xl border border-slate-200 bg-white p-1 text-sm">
                    @can('create_daily_diary')
                        <a href="{{ route('teacher.daily-diary.create', ['session' => $filters['session'] ?? null]) }}" class="rounded-lg px-3 py-1.5 font-semibold text-slate-600 hover:bg-slate-100">Post Diary Entry</a>
                    @endcan
                    <a href="{{ route('teacher.daily-diary.my-entries', ['session' => $filters['session'] ?? null]) }}" class="rounded-lg bg-slate-900 px-3 py-1.5 font-semibold text-white">My Diary Entries</a>
                </div>
            </div>
            @can('create_daily_diary')
                <a
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <form method="GET" action="{{ route('teacher.daily-diary.my-entries') }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                    <div>
                    ...
                    <div class="flex items-end gap-2 md:col-span-2">
                        ...
                        <a href="{{ route('teacher.daily-diary.my-entries') }}" class="inline-flex min-h-11 items-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            Reset
                        </a>
                    </div>
                </form>
